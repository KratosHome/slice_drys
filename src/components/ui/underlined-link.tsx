...
interface IUnderlinedLinkButtonProps extends ButtonHTMLAttributes<HTMLButtonElement> {
  as: 'button'
}

interface IUnderlinedLinkAnchorProps extends AnchorHTMLAttributes<HTMLAnchorElement> {
  as?: 'a'
  href?: string
}
...
>((props, ref) => {
  ...
  const classes: string = cn(
    'group relative flex items-center justify-start gap-3 bg-transparent px-6 py-2 text-[clamp(16px,calc(16px+4*(100vw-375px)/1065),20px)] font-medium transition-all duration-300 ease-in-out motion-reduce:transition-none lg:hover:bg-transparent',
    locale === 'uk'
  ...
  const content = (
    <>
      ...
      <svg
        ...
        className="relative h-[15px] w-[42px] transition-all duration-400 ease-in-out motion-reduce:transition-none lg:group-hover:w-[50px]"
      >
        <path
      ...
      <span className="absolute -bottom-[20px] left-0 w-full px-4 py-2 transition-all duration-300 ease-in-out motion-reduce:transition-none lg:[clip-path:polygon(0%_0%,0%_0%,0%_100%,0%_100%)] lg:group-hover:[clip-path:polygon(0%_0%,100%_0%,100%_100%,0%_100%)]">
